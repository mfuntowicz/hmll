...
struct hmll_fetcher hmll_fetcher_init(struct hmll_context *ctx, const enum hmll_device device)
{
...
size_t hmll_fetch_range(struct hmll_context *ctx, struct hmll_fetcher fetcher, struct hmll_range range, const struct hmll_device_buffer *dst)
{
    if (hmll_has_error(hmll_get_error(ctx)))
        return 0;
...
size_t hmll_fetch_tensor(struct hmll_context *ctx, struct hmll_fetcher fetcher, const char *name, const struct hmll_device_buffer *dst)
{
    if (hmll_has_error(hmll_get_error(ctx)))
        return 0;
